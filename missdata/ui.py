...
def print_help() -> None:
    rows = [
        ...
        ("/map", "Build a no-model-cost project overview and show a suggested test"),
        ("/update [check|apply]", "Check trusted-source updates; apply only after confirmation"),
        ("/export-config [path]", "Export settings only; no keys, logs, history, or sessions"),
        ("/import-config <path>", "Preview and apply a settings-only backup after confirmation"),
        ("/diff [revision]", "Show a bounded read-only Git diff"),
        ("/checkpoint [name]", "Stage and commit all project changes after confirmation"),
        ("/checkpoints, /restore <rev>", "List commits or hard-restore after explicit confirmation"),
        ("/mode <direct|plan>", "Execute immediately or propose a no-tool plan before work"),
        ("/profile <explore|build|review>", "Set local-first work profile without changing provider"),
        ("/review [path]", "Run a read-only code review with only safe inspection tools"),
        ("/privacy [clear <target>]", "View or delete local logs, sessions, and history after confirmation"),
        ("/completion <bash|zsh|fish>", "Print shell completion setup for Miss Data options"),
        ("/sessions, /resume <id>", "List local sessions or resume a selected conversation"),
        ("/session-name <name>, /new-session", "Name the current session or start a saved new one"),
        ("/delete-session <id>", "Delete a local saved conversation after confirmation"),
        ("/approve, /reject", "Execute or discard the currently proposed plan"),
        ("/test [run <n>]", "Discover tests; run one through normal approval controls"),
        ("/context", "Estimate conversation size before a long request"),
        ...
        ("Ctrl+C while generating", "Stop the active response safely; partial turn is not saved"),
        ...
        ("/keys show <provider> [reveal]", "View key slots masked by default; reveal requires confirmation"),
        ("/keys add|replace <provider>", "Append to or replace a provider's key pool using hidden input"),
        ("/keys edit|remove <provider> <n>", "Replace or delete one numbered key slot"),
